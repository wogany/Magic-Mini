...
void CNvs::saveColor(uint8_t r, uint8_t g, uint8_t b)
{
    ...
    /*  获取上次保存的颜色 */
    lastR = PREFS.getUChar("colorR", 0); // 从NVS获取红色分量，默认值为0
    lastG = PREFS.getUChar("colorG", 0); // 从NVS获取绿色分量，默认值为0
    lastB = PREFS.getUChar("colorB", 0); // 从NVS获取蓝色分量，默认值为0
    /* 检测颜色是否变化 */
    if (lastR != r)
    {
        PREFS.putUChar("colorR", r); // 保存红色分量到NVS
    }
    if (lastG != g)
    {
        PREFS.putUChar("colorG", g); // 保存绿色分量到NVS
    }
    if (lastB != b)
    {
        PREFS.putUChar("colorB", b); // 保存蓝色分量到NVS
    }
    PREFS.end();
}
...
void CNvs::loadColor(uint8_t &r, uint8_t &g, uint8_t &b)
{
    ...
    r = PREFS.getUChar("colorR", 255); // 从NVS获取红色分量，默认值为255
    g = PREFS.getUChar("colorG", 255); // 从NVS获取绿色分量，默认值为255
    b = PREFS.getUChar("colorB", 255); // 从NVS获取蓝色分量，默认值为255
    PREFS.end();
}
